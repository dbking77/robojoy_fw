...
static char remote_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = ROBODRIVE_BLE_ADV_NAME;

int g_rssi = 0;

static bool connected  = false;
static bool get_server = false;
static bool ready_for_comm = false;
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ESP_LOGW(GATTC_TAG, "ESP_GATTC_SEARCH_CMPL_EVT");
        ...
        if (get_server){
            ...
            ESP_LOGI(GATTC_TAG, "esp_ble_gattc_get_attr_count %d", count);
            if (count > 0){
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        ESP_LOGW(GATTC_TAG, "ESP_GATTC_REG_FOR_NOTIFY_EVT");
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ...
        */
        RoboDriveCommClient_Notify(esp_timer_get_time(), p_data->notify.value, p_data->notify.value_len);
        break;
    case ESP_GATTC_WRITE_DESCR_EVT:
        ESP_LOGW(GATTC_TAG, "ESP_GATTC_WRITE_DESCR_EVT");
    ...
    case ESP_GATTC_SRVC_CHG_EVT: {
        ESP_LOGW(GATTC_TAG, "ESP_GATTC_SRVC_CHG_EVT");
        esp_bd_addr_t bda;
    ...
    case ESP_GATTC_WRITE_CHAR_EVT:
        //ESP_LOGW(GATTC_TAG, "ESP_GATTC_WRITE_CHAR_EVT");
        if (p_data->write.status != ESP_GATT_OK){
            ESP_LOGE(GATTC_TAG, "Characteristic write failed, status %x)", p_data->write.status);
            break;
        }
        //ESP_LOGW(GATTC_TAG, "p_data->write conn_id %d, handle %d, offset %d",
        //    p_data->write.conn_id, p_data->write.handle, p_data->write.offset);
        //ESP_LOG_BUFFER_HEX(GATTC_TAG, p_data, sizeof(*p_data));
    ...
    case ESP_GATTC_DISCONNECT_EVT:
        ...
        break;
    default:
        ESP_LOGE(GATTC_TAG, "Unhandled event %d", event);
        break;
    }
}
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            if (false)
            {
                // This is very spamy, don't output it for now
    ...
    case ESP_GAP_BLE_READ_RSSI_COMPLETE_EVT:
        //ESP_LOGI(GATTC_TAG, "RSSI read status %d, rssi %d", param->read_rssi_cmpl.status, param->read_rssi_cmpl.rssi);
        if (param->adv_stop_cmpl.status == ESP_BT_STATUS_SUCCESS)
        {
        ...
        //ESP_LOGI(GATTC_TAG, "g_RSSI %d", g_rssi);
        break;
    default:
...
static void esp_gattc_cb(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    //ESP_LOGE(GATTC_TAG, "esp_gattc_cb event %d  param size %d", event, sizeof(esp_ble_gattc_cb_param_t));
...
{
    ESP_ERROR_CHECK(esp_ble_gattc_write_char( gl_profile_tab[PROFILE_A_APP_ID].gattc_if,
...
void GattClient_ReadRSSI()
{
    if (connected)
    {
        ESP_ERROR_CHECK(esp_ble_gap_read_rssi(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
    }
}